...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
    ...
    # open the file
    n = 0
    with open(path, write_mode, newline='', encoding='utf-8-sig') as f:
        writer = csv.writer(f)
        if write_mode == 'w':
        ...
        while until_local <= datetime.datetime.strptime(until, '%Y-%m-%d'):
            ...
            driver, data, writer, tweet_ids, scrolling, tweet_parsed, scroll, last_position, n = \
                keep_scroling(driver, data, writer, tweet_ids, scrolling, tweet_parsed, limit, scroll, last_position, n=n)
